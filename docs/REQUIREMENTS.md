...
| K-3 | Netz-Knoten zeigt den **Betrag** der Netzleistung; der Zustand folgt dem Vorzeichen der Nettoleistung (4.2). | MUSS |
| K-4 | Batterie-Knoten zeigt **zwei Werte**: Ladezustand in `%` **über** dem Icon, Betrag der Lade-/Entladeleistung in `x,xx kW` **unter** dem Icon. Beschriftung „Batterie · lädt" / „Batterie · entlädt" / „Batterie". Der SOC wird über `hass.formatEntityState(stateObj)` formatiert (Nachkommastellen und Leerzeichenregel vor `%` nach HA-Einstellung); Fallback: eigene Formatierung. | MUSS |
...
| V-6 | **Vorbefüllung:** Beim Wechsel in einen Ø-Modus (bzw. beim Laden, wenn `default_mode` ≠ `current`) werden die letzten `avg_long_minutes` Minuten aller beteiligten Entitäten in **einer** Abfrage aus dem Recorder geholt (`history/history_during_period`, `minimal_response`, `no_attributes`, Zeitlimit 5 s), danach läuft der Puffer live weiter. Schlägt das fehl, wird über die verfügbare Zeitspanne gemittelt und mit „Ø 15 min · seit hh:mm" gekennzeichnet. **Keine Vorbefüllung im Editor-Vorschaumodus** (`preview === true`). | MUSS (Kennzeichnung SOLL) |
...
**4.4 Verbraucher.**
1. Konfigurierte Verbraucher mit verfügbarem Wert ≥ `min_consumer_w` → *Kandidaten*.
