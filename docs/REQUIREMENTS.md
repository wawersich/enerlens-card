...
| K-7 | Zahlenformat: **konfigurierbar, einmal für alle Leistungswerte** (`power.unit`: `kW` oder `W`; `power.decimals`: 1–3, nur bei kW; Watt immer ganzzahlig). Standard kW mit genau zwei Nachkommastellen. Rundung kaufmännisch über `Intl.NumberFormat` mit `minimumFractionDigits = maximumFractionDigits = 2` (Grenzfall 1 525 W → `1,53 kW`). Trennzeichen nach `hass.locale` (alle sieben `number_format`-Werte). Negative Null wird nie angezeigt; 0 W → `0,00 kW`. | MUSS |
...
| K-14 | **Gleicher Außendurchmesser aller vier Knoten.** Der Verbraucherring **ersetzt den farbigen Rand** des Haus-Knotens und ist **auf ihm zentriert**: Die Mittellinie des Rings liegt auf der Kreislinie der anderen Knoten, je eine halbe Strichstärke außen und innen. Ohne Ring (aus, keine Verbraucher, oder gerade keine Segmente) hat das Haus den normalen Rand. Entschieden nach Vergleich von sechs Varianten am Gerät: Rand **und** Ring ließen das Haus kleiner wirken (Delboeuf-Effekt), obwohl es messbar gleich groß ist. | MUSS |
| K-12 | **Mindestgrößen auf schmalen Karten.** Bei Kartenbreite ≥ 344 px gilt in CSS-px: Knotenwerte ≥ 12 px, Beschriftungen und Listenzeilen **12 px fest** (dieselbe Größe wie `power-flow-card-plus`, neben der die Karte auf vielen Dashboards steht), SOC-Zahl ≥ 11 px, Punkte 10 px Durchmesser (auf Verbindungen, Fächerlinien und Listenbahnen gleich, wie die Farbmarke der Liste), Verbindungslinien ≥ 3 px, Ring-Strich ≥ 8 px. Umsetzung durch Gegenskalierung anhand der gemessenen Kartenbreite, nicht durch feste viewBox-Einheiten. | MUSS |
...
| P-3 | Geschwindigkeit und Anzahl folgen **einer globalen Stufenregel** mit den Schwellen S1 < S2 < S3 (`flow.slow_below_w`, `flow.more_dots_above_w`, `flow.max_dots_at_w`) und `flow.max_dots`, gleich für alle Verbindungen. **Tempo und Anzahl sind getrennt:** `flow.full_speed_w` (Standard = S2) ist die Leistung, bei der der Punkt sein Höchsttempo erreicht; S2 die, ab der Punkte dazukommen. Liegt `full_speed_w` unter S2, läuft ein einzelner Punkt schnell, bevor der zweite kommt; liegt es darüber, kommen Punkte dazu, während das Tempo noch steigt. Berechnung in 4.6. **Ein Regler `flow.peak_w`** (Standard 6 000 W) setzt alle drei: S1 = peak/12, S2 = peak/3, S3 = peak — ein ausdrücklich gesetzter Wert hat Vorrang. Standard damit: S1 = 500 W, S2 = 2 000 W, S3 = 6 000 W, `max_dots` = 5, `slow_s` = 5, `fast_s` = 1,8. Im Editor: `min_w` und `peak_w` direkt, die sechs Einzelwerte in einem eingeklappten Abschnitt „Feineinstellung"; Eingaben werden nie während des Tippens verändert (E-1). | MUSS |
...
| ID | Anforderung | Prio |
|---|---|---|
| L-1 | Optional (`list.enabled`, Standard: an, sobald `consumers` konfiguriert ist). Rechts vom Haus-Knoten, vertikal auf dessen Höhe, wenn die Inhaltsbreite ≥ **365 px** ist (Kreuz 200 + Liste 145 + Abstand 20); darunter unter dem Kreuz. Freie Breite bekommt zuerst das Kreuz (neben der Liste bis 320 px, **höchstens aber 52 % der Kartenbreite**, mindestens 225 px; steht die Liste unten, bis 400 px), dann die Liste; wird es eng, verliert die Liste (Ellipse im Namen), nie das Kreuz. **Der Wert ist kein Geschmacksurteil:** Home Assistant begrenzt eine Spalte der Sections-Ansicht auf rund 500 px, sodass etwa 460 px Inhalt bleiben — eine höhere Schwelle bedeutet, dass die Liste dort *nie* neben dem Kreuz steht. | MUSS |
| L-2 | Ein Eintrag besteht aus Farbmarke, Name und Wert; **ein konfiguriertes Icon (`icon`) ersetzt die Farbmarke** — an derselben Stelle, in der Eintragsfarbe, 18 px, dort wo die Fächerlinie endet. Kein Rückgriff auf das Icon der Entität: Das `icon`-Attribut ist fast nie gesetzt (die Standard-Icons nach Geräteklasse kennt nur das Frontend), ein Rückgriff zeigte also Icons für einen von zehn Verbrauchern und Punkte für den Rest — geprüft auf der Referenzanlage. Dann folgen Name und Wert `x,xx kW` (K-7). Name aus der Konfiguration, sonst `friendly_name`. Zu lange Namen mit Ellipse gekürzt; der Wert wird nie gekürzt. | MUSS |
...
| L-9 | Klick/Tipp auf einen Eintrag öffnet den More-Info-Dialog der Verbraucher-Entität (I-1). Der **Zeilenabstand richtet sich nach der Anzahl**: bis 4 Einträge 40 px, bis 7 Einträge 34 px, darüber 28 px — drei Einträge dürfen Luft haben, zehn sollen die Karte nicht in die Länge ziehen. Die **Trefferfläche** reicht 5 px über die Zeile hinaus; benachbarte Flächen überlappen sich, sodass **wirksam der Zeilenabstand** die Trefferhöhe ist (28–40 px, stets über der harten Untergrenze 24 px aus I-3; die 44 px aus I-3 gelten für Knoten und Chips, nicht für Listenzeilen — Entscheidung nach Messung, Abschnitt 5). | MUSS |
...
| L-13 | **Linien zur Liste.** Steht die Liste neben dem Kreuz, ist sie nur so breit, wie längster Name und breitester Wert brauchen, und steht am rechten Kartenrand; vom rechten Rand des Haus-Knotens führt zu jedem Eintrag eine Kurve in der Eintragsfarbe über die gesamte freie Breite, mit Punkten nach P-2 — je länger die Linie, desto lesbarer die Bewegung. Steht die Liste unter dem Kreuz, ersetzt eine Bahn am Zeilenanfang die Kurve (kein Fächer über die Zeilen hinweg); die Bahn füllt den Platz zwischen Farbmarke und Namensspalte, Namen und Werte stehen dabei über alle Zeilen in einer Spalte (Grid mit Subgrid; ohne Subgrid feste Bahnbreite). Die Kurven werden in CSS-Pixeln aus gemessenen Positionen gezeichnet und folgen den Zeilen **während** der Gleit-Animation (L-8) Bild für Bild, statt erst am Ende zu springen. | MUSS |
...
| ID | Anforderung | Prio |
|---|---|---|
| E-1 | **Zwei Fehlerpfade — und eine Grundregel: Was der GUI-Editor erzeugen kann, darf die Karte nie zum Werfen bringen** (`test/editor-clear.test.ts` leert jedes Feld so, wie `ha-form` es tut: `""`, `null`, `undefined`). Leer/`null` gilt bei jedem optionalen Wert als „nicht gesetzt". *Reparierbare Widersprüche* werden mit Konsolenwarnung repariert statt abgelehnt: SOC ohne Batterie → ignoriert; Verbraucher ohne Entität oder doppelt → übersprungen; Schwellen in falscher Reihenfolge (`avg_short ≥ avg_long`, `flow.min_w > slow_below_w`, Stufenkette) → der zweite Wert wird angehoben, die Beschriftung zeigt den tatsächlich verwendeten. Der Editor entfernt Leerwerte, verwaiste und doppelte Verbraucher und hält den SOC zurück, bis eine Batterie gewählt ist (E-4) — **korrigiert aber keine Zahlen während der Eingabe**: `ha-form` meldet jeden Tastendruck, und eine Korrektur bei „1" auf dem Weg zu „1000" sperrt das Feld. Reihenfolgen repariert allein die Karte zur Laufzeit. (1) *Strukturfehler* (fehlendes `entities`, unzulässige Mischform, zwei abgeleitete Größen, Bereichsverletzung eines gesetzten Werts) → `setConfig` wirft; HA zeigt die Fehlerkarte. Meldung mit Feldname; Sprache über `hass?.language ?? document.documentElement.lang ?? "en"`, da `hass` zu diesem Zeitpunkt fehlen kann. (2) *Laufzeitprobleme* (Entität fehlt, `unavailable`) → niemals werfen, sondern in der Karte anzeigen (K-9). Unbekannte Schlüssel → Konsolenwarnung. | MUSS |
...
| V-6 | **Vorbefüllung:** Beim Wechsel in einen Ø-Modus (bzw. beim Laden, wenn `default_mode` ≠ `current`) werden die letzten `avg_long_minutes` Minuten aller beteiligten Entitäten in **einer** Abfrage aus dem Recorder geholt (`history/history_during_period`, `minimal_response`, `no_attributes`, Zeitlimit 5 s), danach läuft der Puffer live weiter. Schlägt das fehl, wird über die verfügbare Zeitspanne gemittelt. Solange das Fenster nicht voll ist, trägt der aktive Modus-Chip einen **Tooltip** „seit hh:mm" (hh:mm = Zeitpunkt, ab dem **alle** beteiligten Entitäten Werte haben, also die jüngste erste Probe) — keine sichtbare Textzeile: Die stand als Dauerhinweis im Kopf und störte mehr, als sie half. Mit dem Wechsel auf einen Ø-Modus ist die Vorbefüllung ohnehin nach Sekunden durch. **Keine Vorbefüllung im Editor-Vorschaumodus** (`preview === true`). | MUSS (Kennzeichnung SOLL) |
...
- Tempo: `P ≥ full_speed_w` → `dur = fast_s`; `S1 < P < full_speed_w` → `dur = slow_s − (P − S1)/(full_speed_w − S1) × (slow_s − fast_s)`; sonst `slow_s`.
- Anzahl: `P < S2` → 1; sonst `n = min(max_dots, 2 + floor((P − S2)/(S3 − S2) × (max_dots − 2)))`.
...
- [x] Trefferflächen bei Gerätebreite 360 px und 320 px gemessen (I-3), headless Chromium via `scripts/measure.mjs`, Karte 344 / 304 px: Knoten 94 / 83 px (Haus mit Ring 77 / 70 px), Batterie-Hälften 90 × 67 / 79 × 61 px getrennt, Modus-Chips 36 px hoch mit 8 px Abstand, Filter-Knopf 28 px, Abstand Batterie → erste Zeile 23 px. **Listenzeilen:** wirksame Trefferhöhe = Zeilenabstand (28–40 px je nach Anzahl, L-9), über der harten Untergrenze 24 px; die 44 px gelten dort nicht — entschieden zugunsten der kompakten Liste.
