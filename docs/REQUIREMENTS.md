...
| L-2 | Ein Eintrag besteht aus Farbmarke, Name und Wert; **ein konfiguriertes Icon (`icon`) ersetzt die Farbmarke** — an derselben Stelle, in der Eintragsfarbe, 18 px, dort wo die Fächerlinie endet. Dann folgen Name und Wert `x,xx kW` (K-7). Name aus der Konfiguration, sonst `friendly_name`. Zu lange Namen mit Ellipse gekürzt; der Wert wird nie gekürzt. | MUSS |
