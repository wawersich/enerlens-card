...
| L-13 | **Linien zur Liste.** Steht die Liste neben dem Kreuz, führt vom rechten Rand des Haus-Knotens zu jedem Eintrag eine Kurve in der Eintragsfarbe, mit Punkten nach P-2. Steht die Liste unter dem Kreuz, ersetzt eine Bahn am Zeilenanfang die Kurve (kein Fächer über die Zeilen hinweg); die Bahn füllt den Platz zwischen Farbmarke und Namensspalte, Namen und Werte stehen dabei über alle Zeilen in einer Spalte (Grid mit Subgrid; ohne Subgrid feste Bahnbreite). Die Kurven werden in CSS-Pixeln aus gemessenen Positionen gezeichnet und folgen den Zeilen **während** der Gleit-Animation (L-8) Bild für Bild, statt erst am Ende zu springen. | MUSS |
